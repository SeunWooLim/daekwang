	...
	<!--container s-->
		<div id="container">
			...
			<div class="category_wrap">
				<form action="newFamilyAdmin.do" method="post">
					<ul class="ul_form1">
						<li>
							<c:choose>
						...
						<li>
							<a href="newFamilyAdmin.do" class="btnform2">전체보기</a>
						</li>
					</ul>
				</form>
			...
			<div class="table_wrap">
				<table>
					...
					<tbody>
						<c:forEach var="newFamily" items="${list }">
							<tr>
								<td><input type="checkbox" name="checkBox" value="${newFamily.BOARD_NO }" id="check2"/><label for="check2"></label> </td>
								<td><%= num-- %></td>
								<!-- 카페24 경로 -->
								<%-- <td><img src="<c:url value="/upload/${newFamily.photoVo.UPLOAD_YYMM }/${newFamily.photoVo.PHOTO_RENAME}"/>"></td> --%>
								<!-- 로컬서버 경로 -->
								<td><img src="<c:url value="/resources/uploadPhoto/${newFamily.photoVo.PHOTO_RENAME}"/>"></td>
								<td>${newFamily.memberVo.MEMBER_NAME }</td>
								<td>${newFamily.BOARD_TITLE }</td>
								<c:set var="text" value="${newFamily.BOARD_CONTENT }"/>		
								<%
								...
								<td>${newFamily.FIRST_INPUT_DATE }</td>
								<td>
									<a href="#" class="btnform5" onclick="updateForm(<%= rowNum %>);">수정</a>
									<%-- 
									<c:url var="delete" value="flowerPhotoAdmin.do">
										<c:param name="BOARD_NO" value="${newFamily.BOARD_NO }" />
										<c:param name="currentPage" value="${currentPage }" />
										<c:param name="deleteFlag" value="Y" />
									</c:url>
									 --%>
									<a class="btnform6" onclick="deleteOne(${newFamily.BOARD_NO });">삭제</a>
								</td>
							</tr>
									...
									<div class="form">
										...
										<ul>
											...
											<li>
												<label>이미지</label>
												<!-- 카페24 경로 -->
												<%-- <img src="<c:url value="/upload/${newFamily.photoVo.UPLOAD_YYMM }/${newFamily.photoVo.PHOTO_RENAME}"/>"> --%>
												<!-- 로컬서버 경로 -->
											...
											<li>
												...
												<textarea id="wPA_context<%= ediNum %>" name="BOARD_CONTENT">${newFamily.BOARD_CONTENT }</textarea>
				...
				<ul class="board_page">
					<li>
						<c:url var="first" value="newFamilyAdmin.do">
					...
					<li>
						<c:url var="prev" value="newFamilyAdmin.do">
							<c:param name="currentPage" value="${currentPage}" />
					...
					<c:forEach var="p" begin="${startPage }" end="${endPage }" step="1">
						<li>
							<c:url var="page" value="newFamilyAdmin.do">
					...
					<li>
						<c:url var="next" value="newFamilyAdmin.do">
							<c:param name="currentPage" value="${currentPage }" />
					...
					<li>
						<c:url var="last" value="newFamilyAdmin.do">
...
<script type="text/javascript">
	...
	function deleteOne(BOARD_NO){
		...
		if(confirm("정말 삭제 하시겠습니까?")){
			location.href="newFamilyAdmin.do?BOARD_NO="+BOARD_NO+"&currentPage="+currentPage+"&deleteFlag="+deleteYN;   
		}
	}
	...
	function deleteAction(){
		...
		if(confirm("정말 삭제 하시겠습니까?")){
		    ...
		    var deleteYN = 'Y';
		    location.href="newFamilyAdmin.do?idx="+checkBox+"&currentPage="+currentPage+"&deleteFlag="+deleteYN;      
		}
	}
